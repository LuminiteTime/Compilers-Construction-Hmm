(module
  ...
  (func $_start (export "_start")
    ...
    (local $b f64)
    (local $c i32)
    (local $explicit_int i32)
    (local $explicit_real f64)
    (local $explicit_bool i32)
    (i32.const 10)
    (local.set $x)
    (f64.const 3.14)
    (local.set $y)
    ...
    (local.set $c)
    (i32.const 42)
    (local.set $explicit_int)
    (f64.const 2.71)
    (local.set $explicit_real)
    (i32.const 0)
    (local.set $explicit_bool)
    (local.get $explicit_bool)
    (call $proc_exit)
  )
