(module
  ...
  (func $_start
    ...
    (local $real_val f64)
    (local $bool_val i32)
    (local $int_to_real f64)
    (local $int_to_bool1 i32)
    (local $int_to_bool2 i32)
    (local $real_to_int i32)
    (local $bool_to_int1 i32)
    (local $bool_to_int2 i32)
    (local $bool_to_real1 f64)
    (local $bool_to_real2 f64)
    (i32.const 42)
    (local.set $int_val)
    (f64.const 3.14)
    (local.set $real_val)
    ...
    (local.set $bool_val)
    (local.get $int_val)
    (local.set $int_to_real)
    (i32.const 5)
    (local.set $int_to_bool1)
    (i32.const 0)
    (local.set $int_to_bool2)
    (local.get $real_val)
    (local.set $real_to_int)
    (i32.const 1)
    (local.set $bool_to_int1)
    (i32.const 0)
    (local.set $bool_to_int2)
    (i32.const 1)
    (local.set $bool_to_real1)
    (i32.const 0)
    (local.set $bool_to_real2)
    (local.get $int_to_real)
    (call $print_int)
    (local.get $int_to_bool1)
    (call $print_int)
    (local.get $int_to_bool2)
    (call $print_int)
    (local.get $real_to_int)
    (call $print_int)
    (local.get $bool_to_int1)
    (call $print_int)
    (local.get $bool_to_real1)
    (call $print_int)
    (i32.const 0)
    (call $proc_exit)
  )
